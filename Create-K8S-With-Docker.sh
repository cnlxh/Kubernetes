  ...
  tasks:
    ...
    - block:
        - name: Deploy CRI-Docker
          apt:
            deb: https://class-git.myk8s.cn/Mirantis/cri-dockerd/releases/download/v0.3.15/cri-dockerd_0.3.15.3-0.ubuntu-focal_amd64.deb
        ...
        - name: Deploy CRI-Docker
          apt:
            deb: https://gh-proxy.com/https://github.com/Mirantis/cri-dockerd/releases/download/v0.3.15/cri-dockerd_0.3.15.3-0.ubuntu-focal_amd64.deb

    - name: modify sandbox image to lxh repo
      shell: |
    ...
    - name: modify image repository
      lineinfile:
        path: kubeadm.yaml
        regexp: 'imageRepo.*'
        line: 'imageRepository: class-k8s.myk8s.cn'
        state: present
      when: "'master' in group_names"
    ...
    - name: set up admin role for root user
      shell: |
        mkdir -p $HOME/.kube
      ...
      when: "'master' in group_names"
    - name: set up admin role for vagrant user
      shell: |
        mkdir -p /home/vagrant/.kube
        cp -i /etc/kubernetes/admin.conf /home/vagrant/.kube/config
        chown vagrant:vagrant /home/vagrant/ -R
        sshpass -p vagrant ssh -A -g -o StrictHostKeyChecking=no vagrant@k8s-worker1 mkdir /home/vagrant/.kube 
        sshpass -p vagrant ssh -A -g -o StrictHostKeyChecking=no vagrant@k8s-worker2 mkdir /home/vagrant/.kube
        sshpass -p vagrant scp /etc/kubernetes/admin.conf vagrant@k8s-worker1:/home/vagrant/.kube/config
        sshpass -p vagrant scp /etc/kubernetes/admin.conf vagrant@k8s-worker2:/home/vagrant/.kube/config
      when: "'master' in group_names"
