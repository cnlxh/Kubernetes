...
k8s-master    Ready                         control-plane   11h   v1.30.0
k8s-woker1   NotReady,SchedulingDisabled   worker          11h   v1.30.0
k8s-woker2   Ready                         worker          11h   v1.30.0
```
...
root@k8s-master:~# kubectl get nodes
NAME          STATUS                        ROLES           AGE   VERSION
k8s-master    Ready,SchedulingDisabled      control-plane   11h   v1.30.0
k8s-woker1   NotReady,SchedulingDisabled   worker          11h   v1.30.0
k8s-woker2   Ready                         worker          11h   v1.30.0
```
...
Given an existing kubernetes cluster running version 1.30.0,upgrade all of the Kubernetes control plane and node components on the master node only to version 1.30.1,Please do not upgrade etcd database.
...
现有的 Kubernetes 集群正在运行的版本是 1.30.0，仅将主节点上的所有 kubernetes 控制面板和组件升级到版本 1.30.1，请勿升级etcd数据库， 另外，在主节点上升级 kubelet 和 kubectl
...
Client Version: version.Info {Major:"1", Minor:"29", GitVersion:"v1.30.0"}
Server Version: version.Info {Major:"1", Minor:"29", GitVersion:"v1.30.0"}

root@k8s-master:~# kubeadm version
kubeadm version: &version.Info {Major:"1", Minor:"29", GitVersion:"v1.30.0"}
```
...
root@k8s-master:~# apt install kubeadm=1.30.1-1.1 -y
Reading package lists... Done
...
```bash
root@k8s-master:~# apt-mark unhold kubeadm
root@k8s-master:~# kubeadm upgrade apply v1.30.1 --etcd-upgrade=false
...
...
root@k8s-master:~# apt install kubelet=1.30.1-1.1 kubectl=1.30.1-1.1 -y
Reading package lists... Done
Building dependency tree
Reading state information... Done
...
root@k8s-master:~# kubectl get nodes
NAME          STATUS                        ROLES           AGE   VERSION
k8s-master    Ready,SchedulingDisabled      control-plane   11h   v1.30.1
k8s-woker1   NotReady,SchedulingDisabled   worker          11h   v1.30.0
k8s-woker2   Ready                         worker          11h   v1.30.0
```
...
root@k8s-master:~# kubectl get nodes
NAME          STATUS                        ROLES           AGE   VERSION
k8s-master    Ready                         control-plane   19m   v1.30.1
k8s-woker1   Ready                         worker          17m   v1.30.0
k8s-woker2   Ready                         worker          16m   v1.30.0
```
...
k8s-master    Ready      control-plane   14h   v1.30.1   ...
k8s-woker1   NotReady   worker          14h   v1.30.0   ...
k8s-woker2   Ready      worker          14h   v1.30.0   disk=spinning
```
...
root@k8s-master:~# kubectl get nodes
NAME          STATUS                        ROLES           AGE    VERSION
k8s-master    Ready,SchedulingDisabled      control-plane   2d1h   v1.30.1
k8s-woker1   NotReady,SchedulingDisabled   worker          2d1h   v1.30.0
k8s-woker2   Ready                         worker          2d1h   v1.30.0
```
