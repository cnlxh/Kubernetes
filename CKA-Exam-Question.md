...
internlpod   1/1     Running   0          5m21s   172.16.245.1   k8s-worker2   <none>           <none>
```
...
编辑deployment，在容器下面添加端口，为了优化速度，镜像用了阿里云，考试会给出具体的镜像，考试用它指定的镜像，例如 image: nginx，这样就可以了
...
创建pod，为了优化速度，镜像用了阿里云，考试会给出具体的镜像，考试用它指定的镜像，例如 image: nginx，这样就可以了
...
创建kucc1 pod，为了优化速度，镜像用了阿里云，考试会给出具体的镜像，考试用它指定的镜像，例如 image: nginx，这样就可以了
...
创建pod来使用pvc，为了优化速度，镜像用了阿里云，考试会给出具体的镜像，考试用它指定的镜像，例如 image: nginx，这样就可以了
...
修改导出的yaml文件，在现有容器上添加volumeMounts参数来挂载logs到/var/log，并添加新容器同样挂载这个位置，不要忘了创建出logs卷，为了优化速度，镜像用了阿里云，考试会给出具体的镜像，考试用它指定的镜像，例如 image: nginx，这样就可以了
