...
cka-master    Ready                         control-plane   11h   v1.25.4
cka-worker1   NotReady,SchedulingDisabled   worker          11h   v1.25.4
cka-worker2   Ready                         worker          11h   v1.25.4
```
# Q2: RBAC
...
root@cka-master:~# kubectl get nodes
NAME          STATUS                        ROLES           AGE   VERSION
cka-master    Ready,SchedulingDisabled      control-plane   11h   v1.25.4
cka-worker1   NotReady,SchedulingDisabled   worker          11h   v1.25.4
cka-worker2   Ready                         worker          11h   v1.25.4
```
将现有工作负载驱赶到其他节点
...
Given an existing kubernetes cluster running version 1.25.4,upgrade all of the Kubernetes control plane and node components on the master node only to version 1.25.5,Please do not upgrade etcd database.
...
现有的 Kubernetes 集权正在运行的版本是 1.25.4，仅将主节点上的所有 kubernetes 控制面板和组件升级到版本 1.25.5，请勿升级etcd数据库， 另外，在主节点上升级 kubelet 和 kubectl
...
root@cka-master:~# kubeadm version
kubeadm version: &version.Info{Major:"1", Minor:"25", GitVersion:"v1.25.5"}
```
查询现有可升级的版本
```bash
...
```bash
root@cka-master:~# apt-mark unhold kubeadm
root@cka-master:~# kubeadm upgrade apply v1.25.5 --etcd-upgrade=false
...
...
root@cka-master:~# apt install kubelet=1.25.5-00 kubectl=1.25.5-00 -y
Reading package lists... Done
Building dependency tree
Reading state information... Done
root@cka-master:~# apt-mark hold kubeadm
...
root@cka-master:~# kubectl get nodes
NAME          STATUS                        ROLES           AGE   VERSION
cka-master    Ready,SchedulingDisabled      control-plane   11h   v1.25.5
cka-worker1   NotReady,SchedulingDisabled   worker          11h   v1.25.4
cka-worker2   Ready                         worker          11h   v1.25.4
```
...
root@cka-master:~# kubectl get nodes
NAME          STATUS                        ROLES           AGE   VERSION
cka-master    Ready                         control-plane   19m   v1.25.5
cka-worker1   Ready                         worker          17m   v1.25.4
cka-worker2   Ready                         worker          16m   v1.25.4
```
...
cka-master    Ready      control-plane   14h   v1.25.5   ...
cka-worker1   NotReady   worker          14h   v1.25.4   ...
cka-worker2   Ready      worker          14h   v1.25.4   disk=spinning
```
查询pod是否如期调度
```bash
...
root@cka-master:~# kubectl get nodes
NAME          STATUS                        ROLES           AGE    VERSION
cka-master    Ready,SchedulingDisabled      control-plane   2d1h   v1.25.5
cka-worker1   NotReady,SchedulingDisabled   worker          2d1h   v1.25.4
cka-worker2   Ready                         worker          2d1h   v1.25.4
```
