...
cka-master    Ready                         control-plane   11h   v1.25.5
cka-worker1   NotReady,SchedulingDisabled   worker          11h   v1.25.5
cka-worker2   Ready                         worker          11h   v1.25.5
```
# Q2: RBAC
...
Given an existing kubernetes cluster running version 1.25.5,upgrade all of the Kubernetes control plane and node components on the master node only to version 1.26.0,Please do not upgrade etcd database.
...
Client Version: version.Info{Major:"1", Minor:"25", GitVersion:"v1.25.5"}
Server Version: version.Info{Major:"1", Minor:"25", GitVersion:"v1.25.5"}
...
root@cka-master:~# apt install kubeadm=1.26.0-00 -y
Reading package lists... Done
...
```bash
root@cka-master:~# apt-mark unhold kubeadm
root@cka-master:~# kubeadm upgrade apply v1.26.0 --etcd-upgrade=false
...
...
root@cka-master:~# kubectl get nodes
NAME          STATUS                        ROLES           AGE   VERSION
cka-master    Ready                         control-plane   19m   v1.26.0
cka-worker1   Ready                         worker          17m   v1.25.5
cka-worker2   Ready                         worker          16m   v1.25.5
```
...
cka-master    Ready      control-plane   14h   v1.26.0   ...
cka-worker1   NotReady   worker          14h   v1.25.5   ...
cka-worker2   Ready      worker          14h   v1.25.5   disk=spinning
```
查询pod是否如期调度
```bash
...
root@cka-master:~# kubectl get nodes
NAME          STATUS                        ROLES           AGE    VERSION
cka-master    Ready,SchedulingDisabled      control-plane   2d1h   v1.26.0
cka-worker1   NotReady,SchedulingDisabled   worker          2d1h   v1.25.5
cka-worker2   Ready                         worker          2d1h   v1.25.5
```
