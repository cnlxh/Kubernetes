...
function multicontainer {
  ...
  if kubectl get pod kucc1 -o yaml 2> /dev/null | grep -E -qi image:.*nginx && kubectl get pod kucc1 -o yaml 2> /dev/null | grep -q image:.*redis && \
     kubectl get pod kucc1 -o yaml 2> /dev/null | grep -qi image:.*memcached && kubectl get pod kucc1 -o yaml 2> /dev/null | grep -q image:.*registry;then
    score=$(expr $score + 2 )
    pass && echo 'kucc1 pod已经同时包含registry、nginx、memcached、redis'
  else
    fail && echo 'kucc1 pod必须同时包含registry、nginx、memcached、redis'
  fi
  echo
  echo
}
