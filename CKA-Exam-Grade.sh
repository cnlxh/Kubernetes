...
function upgrade {
  ...
  echo
  if kubectl get nodes | grep k8s-master | grep -q 1.30.1;then
    score=$(expr $score + 2 )
    pass && echo 'k8s-master已升级到1.30.1'
  else
    fail && echo 'k8s-master没有成功升级到1.30.1'
  fi  
  if kubectl version 2> /dev/null | grep -q v1.30.1 &> /dev/null &> /dev/null;then
    score=$(expr $score + 2 )
    pass && echo 'kubectl已升级到1.30.1'
  else
    fail && echo 'kubectl没有成功升级到1.30.1'
  fi  
  if kubelet --version | grep -q v1.30.1 &> /dev/null;then
    score=$(expr $score + 2 )
    pass && echo 'kubelet已升级到1.30.1'
  else
    fail && echo 'kubelet没有成功升级到1.30.1'
  fi  
  echo
  echo  
}
