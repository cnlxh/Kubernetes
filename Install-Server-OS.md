...
点击vmnet8，并确保vmnet8是nat类型，输入子网192.168.30.0，掩码255.255.255.0
...
用上面自己的用户名和密码登录后，输入sudo -i回车，输入自己的密码，并输入passwd root，给root用户设置密码，请注意，所有虚拟机的root密码，必须是数字1
...
```bash
https://download.mobatek.net/2222022102210348/MobaXterm_Portable_v22.2.zip
```


# 设置永久静态IP地址

请通过SSH工具链接，然后执行下面的内容

先确定接口名称，这里发现接口名称为ens33
...
![start](https://gitee.com/cnlxh/Kubernetes/raw/master/images/system/server/ipadd.png)


# 配置OpenSSH-Server

开通root用户的ssh权限以方便我们远程，此为课程所必须，请确保3台虚拟机上都配置好了root的ssh登录，且密码为1
