  ...
  tasks:
    ...
    - block:
        - name: Download Containerd on master node
          get_url:
            url: https://class-git.myk8s.cn/containerd/nerdctl/releases/download/v1.7.7/nerdctl-full-1.7.7-linux-amd64.tar.gz
            dest: /root/nerdctl-full-1.7.7-linux-amd64.tar.gz
          register: result
        ...
        - name: Download Containerd on master node again
          get_url:
            url: https://gh-proxy.com/https://github.com/containerd/nerdctl/releases/download/v1.7.7/nerdctl-full-1.7.7-linux-amd64.tar.gz
    ...
    - name: Copy Containerd package to node
      copy:
        src: /root/nerdctl-full-1.7.7-linux-amd64.tar.gz
        dest: /root/nerdctl-full-1.7.7-linux-amd64.tar.gz
        force: yes
    ...
    - name: Update SystemdCgroup to true
      lineinfile:
        ...
        backrefs: yes
    - name: Update sandbox_image
      lineinfile:
        path: /etc/containerd/config.toml
        regexp: '^(\s*)sandbox_image = "registry.k8s.io/pause:3.8"'
        line: '\1sandbox_image = "registry.cn-hangzhou.aliyuncs.com/google_containers/pause:3.10"'
        backrefs: yes
    - name: Configure docker hub mirror and bash_completion
      shell: |
    ...
    - name: Create directory for Docker hub mirror
      file:
    ...
    - name: install kubeadm kubectl kubelet
      package:
        name:
          ...
          - sshpass
        state: present
    - name: integrate with containerd
    ...
    - name: Modify criSocket to containerd
      lineinfile:
        path: kubeadm.yaml
        regexp: '(^\s*)criSocket.*'
        line: '\1criSocket: unix:///run/containerd/containerd.sock'
        backrefs: yes
      when: "'master' in group_names"
    - name: restart containerd kubelet service
      systemd:
        state: restarted
        daemon_reload: yes
        name: "{{ item }}"
    ...
    - name: join workers
      shell: |
        ...
        echo $join --cri-socket=unix:///run/containerd/containerd.sock | bash
      register: result
      until: result.rc == 0
      retries: 5
      delay: 2
...
rm -rf create-k8s.yaml /root/ansible.cfg /root/kubeadm.yaml /root/Create-K8S-With-Containerd.sh /tmp/installdetails.log nerdctl* wget-log
sshpass -p vagrant ssh -A -g -o StrictHostKeyChecking=no root@k8s-worker1 'rm -rf create-k8s.yaml /root/ansible.cfg /root/kubeadm.yaml /root/Create-K8S-With-Containerd.sh /tmp/installdetails.log nerdctl* wget-log'
sshpass -p vagrant ssh -A -g -o StrictHostKeyChecking=no root@k8s-worker2 'rm -rf create-k8s.yaml /root/ansible.cfg /root/kubeadm.yaml /root/Create-K8S-With-Containerd.sh /tmp/installdetails.log nerdctl* wget-log'
